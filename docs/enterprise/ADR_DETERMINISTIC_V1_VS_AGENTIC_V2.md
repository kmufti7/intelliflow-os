...
- **Deterministic kill-switch principle.** Whether an operator can disable LLM involvement in the decision pipeline without losing decision-making capability.
...
IntelliFlow OS v1 enforces a strict boundary: LLMs are data extraction and explanation formatting tools. They do not make decisions. This principle is not a preference — it is a compliance expectation for deployments in healthcare (HIPAA audit trails) and banking (SR 11-7 model risk management alignment).
...
| **Regulatory risk** | Very high. All existing compliance documentation, test suites (253 tests), and enterprise evidence (27 docs, 153 checks) apply to v1 architecture. Rewrite invalidates all of it. |
| **Customer impact** | Operators lose access to deterministic modules. No migration path — only replacement. |

**Rejection rationale:** Destroys 253 tests, 27 enterprise documents, and 153 verification checks worth of validated compliance infrastructure. The rewrite provides no incremental regulatory benefit over Option B and introduces significant regression risk.
...
| **Enterprise evidence reuse** | Full. All 27 docs and 153 checks apply directly. | Partial. Governance framework (NIST AI RMF, OWASP) applies. Module-specific evidence requires new documentation. |

---

## Applicability Matrix — When to Use v1 vs. v2

| Use Case | Recommended Version | Reasoning | Risk Profile |
|----------|---------------------|-----------|-------------|
| Clinical gap detection, lab value extraction, severity classification | **v1 (Deterministic)** | Decision logic must be auditable as Python code. Regulators expect deterministic output for clinical recommendations. Kill-switch required. | Low model risk — LLM isolated to extraction/explanation. |
| Policy-grounded customer support routing, compliance citation | **v1 (Deterministic)** | Enum-constrained classification and keyword-based retrieval are fully inspectable. No probabilistic routing in the decision path. | Low model risk — routing is deterministic code. |
| Multi-step claims processing with external registry lookups | **v2 (Agentic)** | Workflow requires tool orchestration (MCP-scoped registry_lookup), conditional halting (KillSwitchGuard), and multi-node state propagation — patterns native to LangGraph. | Moderate — guardrails (KillSwitchGuard, MCPRegistry) bound the decision space. |
| Workflows requiring dynamic tool selection per node | **v2 (Agentic)** | MCPRegistry provides per-node tool scoping. Static pipelines cannot adapt tool availability based on workflow state. | Moderate — Least-Privilege scoping reduces blast radius. |
| Any workflow where LLM disablement must preserve full decision capability | **v1 (Deterministic)** | Kill-switch principle: disabling the LLM loses only explanation formatting, not decision logic. v2 cannot provide this guarantee. | Lowest — zero model risk when LLM disabled. |
| Workflows where speed-to-production outweighs audit trail determinism | **v2 (Agentic)** | Agent configuration (hours-to-days) vs. pipeline implementation (days-to-weeks). Governance guardrails provide bounded compliance. | Higher — requires guardrail validation and scenario-based testing. |

**Selection principle:** Default to v1 for any workflow where a regulator, auditor, or compliance officer will ask "show me the code that made this decision." Use v2 when the workflow requires multi-step orchestration with tool selection and the compliance posture can be satisfied by guardrail-bounded agent behavior with structured audit trails.
...
- **Test and documentation investment is protected.** 253 tests and 27 enterprise documents remain valid for v1. v2 builds on the governance framework without invalidating existing evidence.
