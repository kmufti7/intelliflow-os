...
def generate_tests_for_schema(schema_cls) -> str:
    ...
    kwargs_str = ", ".join(f'{k}={v}' for k, v in required_kwargs.items())
    ...
    for name, info in fields.items():
        if "ge" in info["constraints"]:
            ...
            lines.append(f"    event = {class_name}({kwargs_str.replace(f'{name}={required_kwargs.get(name, '1')}', f'{name}={boundary}')})")
            ...
            lines.append(f"    with pytest.raises(ValidationError):")
            lines.append(f"        {class_name}({kwargs_str.replace(f'{name}={required_kwargs.get(name, '1')}', f'{name}={below}')})")
            lines.append("")
    ...
    for name, info in fields.items():
        if info["is_optional"]:
            ...
            lines.append(f"    event = {class_name}({kwargs_str}, {name}=None)")
